...
#[derive(Clone, Debug)]
pub enum NodeEvent {
    /// The node is dialing an initial peer and attempting to connect to the network
    AttemptingNetworkConnection,
    /// The node's attempt to dial the initial peers and connect to the network is timing out
